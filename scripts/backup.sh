...
BACKUP_FILENAME="caloscanai_backup_${DATE}.dump"
...
# 執行備份
echo "正在備份資料庫..."
docker exec ${CONTAINER_NAME} pg_dump -U caloscanai -d caloscanai -Fc -f /tmp/${BACKUP_FILENAME}
...
find "${BACKUP_DIR}" -name "caloscanai_backup_*.dump.gz" -mtime +${RETENTION_DAYS} -delete
...
echo ""
echo "📋 目前備份檔案列表:"
ls -lh "${BACKUP_DIR}"/caloscanai_backup_*.dump.gz 2>/dev/null | tail -5
